...
char* isKW(char* word) {
    kr_str("");
    if (kr_truthy(kr_eq(word, kr_str("struct")))) {
        return kr_str("1");
...
char* compileStmt(char* tokens, char* pos, char* ntoks, char* depth, char* inFunc) {
    ...
    if (kr_truthy(kr_eq(tok, kr_str("KW:module")))) {
        char* p = kr_plus(pos, kr_str("1"));
        return kr_plus(kr_plus(indent(depth), kr_str("// module declaration\n,")), p);
    }
    if (kr_truthy(kr_eq(tok, kr_str("KW:import")))) {
        char* p = kr_plus(pos, kr_str("1"));
        return kr_plus(kr_plus(indent(depth), kr_str("// import statement\n,")), p);
    }
    if (kr_truthy(kr_eq(tok, kr_str("KW:export")))) {
        char* p = kr_plus(pos, kr_str("1"));
        return kr_plus(kr_plus(indent(depth), kr_str("// export statement\n,")), p);
    }

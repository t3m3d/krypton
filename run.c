...
char* evalCall(char* tokens, char* pos, char* env, char* ftable) {
    ...
    if (kr_truthy(kr_eq(fname, kr_str("sbNew")))) {
        return kr_plus(kr_plus(kr_sbnew(), kr_str(",")), p);
    }
    if (kr_truthy(kr_eq(fname, kr_str("sbAppend")))) {
        char* sb = getArg(args, kr_str("0"));
        char* s = getArg(args, kr_str("1"));
        return kr_plus(kr_plus(kr_sbappend(sb, s), kr_str(",")), p);
    }
    if (kr_truthy(kr_eq(fname, kr_str("sbToString")))) {
        char* sb = getArg(args, kr_str("0"));
        return kr_plus(kr_plus(kr_sbtostring(sb), kr_str(",")), p);
    }

...
just run {
    ...
    // Runtime trees
    exec("cp -R stdlib \"" + rootd + "/stdlib\"")
    exec("cp -R headers \"" + rootd + "/headers\"")
    if isDir("examples") == "yes" { exec("cp -R examples \"" + rootd + "/examples\"") }
    ...
    let inst = "#!/usr/bin/env bash\n" +
        ...
        "for b in bootstrap/kcc_driver_macos_aarch64 compiler/macos_arm64/kcc-arm64 compiler/macos_arm64/macho_host compiler/macos_arm64/kls; do\n" +
    ...
    exec("find \"" + rootd + "\" -type f \\( -name macho_host -o -name 'kcc-*' -o -name 'kcc_*' \\) -exec touch {} +")
    exec("sleep 1")
    exec("find \"" + rootd + "\" -type f \\( -name macho_host -o -name 'kcc-*' -o -name 'kcc_*' \\) -exec touch {} +")

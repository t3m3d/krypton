...
char* unwrap(char*);
char* unwrapErr(char*);
char* unwrapOr(char*, char*);
char* resultMap(char*, char*);
char* name(char*, char*);
// --- imported: stdlib/math_utils.k ---
char* k_abs(char*);
char* max(char*, char*);
char* min(char*, char*);
...
char* fibonacci(char*);
char* digitSum(char*);
char* digitCount(char*);
char* clamp(char*, char*, char*);
char* isqrt(char*);
char* sign(char*);
char* sumTo(char*);
// --- imported: stdlib/json.k ---
char* jsonStr(char*);
char* jsonNum(char*);
char* jsonBool(char*);
char* jsonNull();
...
char* ok(char* val) {
    return kr_plus(kr_str("ok:"), val);
}

char* err(char* msg) {
    return kr_plus(kr_str("err:"), msg);
}

char* isOk(char* result) {
    return kr_startswith(result, kr_str("ok:"));
}

char* isErr(char* result) {
    return kr_startswith(result, kr_str("err:"));
}

char* unwrap(char* result) {
    if (kr_truthy(kr_startswith(result, kr_str("ok:")))) {
        return kr_substr(result, kr_str("3"), kr_len(result));
    }
    kr_print(kr_plus(kr_str("ERROR: unwrap called on Err result: "), result));
    kr_exit(kr_str("1"));
    return kr_str("");
}

char* unwrapErr(char* result) {
    if (kr_truthy(kr_startswith(result, kr_str("err:")))) {
        return kr_substr(result, kr_str("4"), kr_len(result));
    }
    kr_print(kr_plus(kr_str("ERROR: unwrapErr called on Ok result: "), result));
    kr_exit(kr_str("1"));
    return kr_str("");
}

char* unwrapOr(char* result, char* k_default) {
    if (kr_truthy(kr_startswith(result, kr_str("ok:")))) {
        return kr_substr(result, kr_str("3"), kr_len(result));
    }
    return k_default;
...
char* resultMap(char* result, char* name) {
    if (kr_truthy(kr_startswith(result, kr_str("err:")))) {
        return result;
    }
    char* val = kr_substr(result, kr_str("3"), kr_len(result));
    return ok(val);
}

char* k_abs(char* n) {
    if (kr_truthy(kr_lt(n, kr_str("0")))) {
        return kr_sub(kr_str("0"), n);
    }
    return n;
}

char* max(char* a, char* b) {
    if (kr_truthy(kr_gt(a, b))) {
        return a;
    }
    return b;
}
...
char* power(char* base, char* exp) {
    char* result = kr_str("1");
    char* i = kr_str("0");
    while (kr_truthy(kr_lt(i, exp))) {
        result = kr_mul(result, base);
...
char* factorial(char* n) {
    if (kr_truthy(kr_lte(n, kr_str("1")))) {
        return kr_str("1");
    }
    return kr_mul(n, factorial(kr_sub(n, kr_str("1"))));
}

char* gcd(char* a, char* b) {
    char* x = kr_abs(a);
    char* y = kr_abs(b);
    while (kr_truthy(kr_neq(y, kr_str("0")))) {
        char* temp = y;
        y = kr_mod(x, temp);
        x = temp;
    }
...
char* lcm(char* a, char* b) {
    if (kr_truthy(kr_eq(a, kr_str("0")))) {
        return kr_str("0");
    }
    if (kr_truthy(kr_eq(b, kr_str("0")))) {
        return kr_str("0");
    }
    return kr_div(kr_abs(kr_mul(a, b)), gcd(a, b));
}

char* isPrime(char* n) {
    if (kr_truthy(kr_lt(n, kr_str("2")))) {
        return kr_str("0");
    }
    if (kr_truthy(kr_eq(n, kr_str("2")))) {
        return kr_str("1");
    }
    if (kr_truthy(kr_eq(kr_mod(n, kr_str("2")), kr_str("0")))) {
        return kr_str("0");
    }
    char* i = kr_str("3");
    while (kr_truthy(kr_lte(kr_mul(i, i), n))) {
        if (kr_truthy(kr_eq(kr_mod(n, i), kr_str("0")))) {
            return kr_str("0");
        }
...
char* fibonacci(char* n) {
    if (kr_truthy(kr_lte(n, kr_str("0")))) {
        return kr_str("0");
    }
    if (kr_truthy(kr_eq(n, kr_str("1")))) {
        return kr_str("1");
    }
    char* a = kr_str("0");
    char* b = kr_str("1");
    char* i = kr_str("2");
    while (kr_truthy(kr_lte(i, n))) {
        char* temp = kr_plus(a, b);
        a = b;
        b = temp;
        i = kr_plus(i, kr_str("1"));
    }
...
char* digitSum(char* n) {
    char* num = kr_abs(n);
    char* sum = kr_str("0");
    while (kr_truthy(kr_gt(num, kr_str("0")))) {
        sum = kr_plus(sum, kr_mod(num, kr_str("10")));
        num = kr_div(num, kr_str("10"));
    }
    return sum;
}

char* digitCount(char* n) {
    if (kr_truthy(kr_eq(n, kr_str("0")))) {
        return kr_str("1");
    }
    char* num = kr_abs(n);
    char* cnt = kr_str("0");
    while (kr_truthy(kr_gt(num, kr_str("0")))) {
        cnt = kr_plus(cnt, kr_str("1"));
        num = kr_div(num, kr_str("10"));
    }
    return cnt;
}

char* clamp(char* val, char* lo, char* hi) {
    if (kr_truthy(kr_lt(val, lo))) {
        return lo;
    }
    if (kr_truthy(kr_gt(val, hi))) {
        return hi;
    }
    return val;
}
...
char* isqrt(char* n) {
    ...
    char* x = n;
    char* y = kr_div(kr_plus(x, kr_str("1")), kr_str("2"));
    while (kr_truthy(kr_lt(y, x))) {
        x = y;
        y = kr_div(kr_plus(x, kr_div(n, x)), kr_str("2"));
    }
    return x;
}
...
char* sumTo(char* n) {
    return kr_div(kr_mul(n, kr_plus(n, kr_str("1"))), kr_str("2"));
}

char* jsonStr(char* s) {
    return kr_plus(kr_plus(kr_str("\""), kr_replace(kr_replace(s, kr_str("\\"), kr_str("\\\\")), kr_str("\""), kr_str("\\\""))), kr_str("\""));
}
...
char* jsonBool(char* b) {
    if (kr_truthy((kr_truthy(kr_eq(b, kr_str("1"))) || kr_truthy(kr_eq(b, kr_str("true"))) ? kr_str("1") : kr_str("0")))) {
        return kr_str("true");
    }
    return kr_str("false");
}
...
char* jsonArray(char* lst) {
    if (kr_truthy(kr_eq(kr_len(lst), kr_str("0")))) {
        return kr_str("[]");
    }
    char* sb = kr_sbnew();
    sb = kr_sbappend(sb, kr_str("["));
    char* cnt = kr_length(lst);
    char* i = kr_str("0");
    {
        char* _for_col_1 = lst;
        int _for_cnt_1 = kr_listlen(_for_col_1);
        for (int _for_i_1 = 0; _for_i_1 < _for_cnt_1; _for_i_1++) {
            char* item = kr_split(_for_col_1, kr_itoa(_for_i_1));
        if (kr_truthy(kr_gt(i, kr_str("0")))) {
            sb = kr_sbappend(sb, kr_str(","));
        }
        sb = kr_sbappend(sb, jsonStr(item));
        i = kr_plus(i, kr_str("1"));
        }
    }
    sb = kr_sbappend(sb, kr_str("]"));
...
char* jsonObject(char* map) {
    char* sb = kr_sbnew();
    sb = kr_sbappend(sb, kr_str("{"));
    char* ks = kr_keys(map);
    char* cnt = kr_length(ks);
    char* i = kr_str("0");
    {
        char* _for_col_1 = ks;
        int _for_cnt_1 = kr_listlen(_for_col_1);
        for (int _for_i_1 = 0; _for_i_1 < _for_cnt_1; _for_i_1++) {
            char* k = kr_split(_for_col_1, kr_itoa(_for_i_1));
        if (kr_truthy(kr_gt(i, kr_str("0")))) {
            sb = kr_sbappend(sb, kr_str(","));
        }
        sb = kr_sbappend(sb, kr_plus(kr_plus(jsonStr(k), kr_str(":")), jsonStr(kr_mapget(map, k))));
        i = kr_plus(i, kr_str("1"));
        }
    }
    sb = kr_sbappend(sb, kr_str("}"));
...
char* safeSqrt(char*);
char* buildStats(char*);

char* safeSqrt(char* n) {
    if (kr_truthy(kr_lt(n, kr_str("0")))) {
        return err(kr_str("cannot take sqrt of negative number"));
    }
    return ok(kr_plus(kr_sqrt(kr_toint(n)), kr_str("")));
}

char* buildStats(char* numbers) {
    char* m = kr_str("");
    m = kr_mapset(m, kr_str("count"), kr_plus(kr_length(numbers), kr_str("")));
    m = kr_mapset(m, kr_str("sum"), kr_plus(kr_sumlist(numbers), kr_str("")));
    m = kr_mapset(m, kr_str("min"), kr_plus(kr_minlist(numbers), kr_str("")));
    m = kr_mapset(m, kr_str("max"), kr_plus(kr_maxlist(numbers), kr_str("")));
    m = kr_mapset(m, kr_str("sorted"), kr_sort(numbers));
    return m;
}
...
int main(int argc, char** argv) {
    ...
    kr_print(kr_str("=== Module System Demo ==="));
    char* r1 = safeSqrt(kr_str("144"));
    char* r2 = safeSqrt(kr_str("-5"));
    if (kr_truthy(isOk(r1))) {
        kr_print(kr_cat(kr_str("sqrt(144) = "), unwrap(r1)));
    }
    if (kr_truthy(isErr(r2))) {
        kr_print(kr_cat(kr_str("error: "), unwrapErr(r2)));
    }
    kr_print(kr_cat(kr_str("gcd(48, 18) = "), gcd(kr_str("48"), kr_str("18"))));
    kr_print(kr_cat(kr_str("isPrime(17) = "), isPrime(kr_str("17"))));
    kr_print(kr_cat(kr_str("fibonacci(10) = "), fibonacci(kr_str("10"))));
    char* stats = buildStats(kr_str("5,3,9,1,7,2,8,4,6"));
    kr_print(kr_str(""));
    kr_print(kr_str("Stats as JSON:"));
    kr_print(jsonObject(stats));
    char* nums = kr_cat(kr_cat(kr_cat(kr_cat(kr_cat(kr_cat(kr_cat(kr_cat(kr_str("4"), kr_str(",")), kr_str("9")), kr_str(",")), kr_str("16")), kr_str(",")), kr_str("25")), kr_str(",")), kr_str("36"));
    kr_print(kr_str(""));
    kr_print(kr_str("Square roots:"));
    {
        char* _for_col_1 = nums;
        int _for_cnt_1 = kr_listlen(_for_col_1);
        for (int _for_i_1 = 0; _for_i_1 < _for_cnt_1; _for_i_1++) {
            char* n = kr_split(_for_col_1, kr_itoa(_for_i_1));
        char* r = safeSqrt(n);
        kr_print(kr_cat(kr_cat(kr_cat(kr_str("  sqrt("), n), kr_str(") = ")), unwrap(r)));
        }
    }
    return 0;
}

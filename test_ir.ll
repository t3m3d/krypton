...
char* irGetArg2(char*);
char* irGetRest(char*);
char* llvmSanitize(char*);
char* emitGlobalsForFunc(char*, char*);
char* emitFunc(char*, char*, char*);

char* irGetOp(char* line) {
    char* t = kr_trim(line);
    char* i = kr_str("0");
    while (kr_truthy((kr_truthy(kr_lt(i, kr_len(t))) && kr_truthy(kr_neq(kr_idx(t, kr_atoi(i)), kr_str(" "))) ? kr_str("1") : kr_str("0")))) {
        i = kr_plus(i, kr_str("1"));
    }
    return kr_substr(t, kr_str("0"), i);
}
...
char* irGetArg1(char* line) {
    ...
    if (kr_truthy(kr_gte(i, kr_len(t)))) {
        return kr_str("");
    }
    i = kr_plus(i, kr_str("1"));
    char* start = i;
    ...
    return kr_trim(kr_substr(t, start, i));
}
...
char* irGetArg2(char* line) {
    ...
    char* i = kr_str("0");
    char* sp = kr_str("0");
    while (kr_truthy(kr_lt(i, kr_len(t)))) {
        if (kr_truthy(kr_eq(kr_idx(t, kr_atoi(i)), kr_str(" ")))) {
            sp = kr_plus(sp, kr_str("1"));
            if (kr_truthy(kr_eq(sp, kr_str("2")))) {
                return kr_trim(kr_substr(t, kr_plus(i, kr_str("1")), kr_len(t)));
            }
        }
        i = kr_plus(i, kr_str("1"));
    }
...
char* llvmSanitize(char* s) {
    ...
    while (kr_truthy(kr_lt(i, kr_len(s)))) {
        ...
        if (kr_truthy(kr_eq(c, kr_str("\\")))) {
            ...
            result = kr_plus(result, kr_str("\\22"));
        } else if (kr_truthy(kr_eq(c, kr_str("\n")))) {
            result = kr_plus(result, kr_str("\\0A"));
        } else if (kr_truthy(kr_eq(c, kr_str("\t")))) {
            result = kr_plus(result, kr_str("\\09"));
        } else {
            result = kr_plus(result, c);
        }
        i = kr_plus(i, kr_str("1"));
    }
    return result;
...
char* emitGlobalsForFunc(char* fname, char* body) {
    char* out = kr_str("");
    char* lines = body;
    char* count = kr_linecount(lines);
    char* pushIdx = kr_str("0");
    char* i = kr_str("0");
    while (kr_truthy(kr_lt(i, count))) {
        char* line = kr_getline(lines, i);
        char* op = irGetOp(line);
        if (kr_truthy(kr_eq(op, kr_str("PUSH")))) {
            char* val = kr_trim(irGetRest(line));
            if (kr_truthy(kr_startswith(val, kr_str("\"")))) {
                char* inner = kr_substr(val, kr_str("1"), kr_sub(kr_len(val), kr_str("1")));
                char* slen = kr_plus(kr_len(inner), kr_str("1"));
                char* san = llvmSanitize(inner);
                out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("@kp_")), fname), kr_str("_")), pushIdx), kr_str(" = private unnamed_addr constant [")), slen), kr_str(" x i8] c\"")), san), kr_str("\\00\"\n"));
            } else {
                char* slen = kr_plus(kr_len(val), kr_str("1"));
                out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("@kp_")), fname), kr_str("_")), pushIdx), kr_str(" = private unnamed_addr constant [")), slen), kr_str(" x i8] c\"")), val), kr_str("\\00\"\n"));
            }
            pushIdx = kr_plus(pushIdx, kr_str("1"));
        }
        i = kr_plus(i, kr_str("1"));
    }
    return out;
...
char* emitFunc(char* fname, char* argc, char* body) {
    char* paramCount = kr_toint(argc);
    char* out = kr_str("");
    out = kr_plus(kr_plus(kr_plus(out, kr_str("define ptr @")), fname), kr_str("("));
    char* pi = kr_str("0");
    while (kr_truthy(kr_lt(pi, paramCount))) {
        if (kr_truthy(kr_gt(pi, kr_str("0")))) {
            out = kr_plus(out, kr_str(", "));
        }
        out = kr_plus(kr_plus(out, kr_str("ptr %p")), pi);
        pi = kr_plus(pi, kr_str("1"));
    }
    out = kr_plus(out, kr_str(") {\nentry:\n"));
    char* si = kr_str("0");
    while (kr_truthy(kr_lt(si, kr_str("32")))) {
        out = kr_plus(kr_plus(kr_plus(out, kr_str("  %stk")), si), kr_str(" = alloca ptr\n"));
        si = kr_plus(si, kr_str("1"));
    }
    out = kr_plus(out, kr_str("  %_empty_slot = alloca ptr\n"));
    char* lines = body;
    char* count = kr_linecount(lines);
    char* stkDepth = kr_str("0");
    char* reg = kr_str("0");
    char* paramIdx = kr_str("0");
    char* gidx = kr_str("0");
    char* lastWasTerm = kr_str("0");
    char* i = kr_str("0");
    while (kr_truthy(kr_lt(i, count))) {
        char* line = kr_getline(lines, i);
        char* op = irGetOp(line);
        if (kr_truthy(kr_eq(op, kr_str("PARAM")))) {
            char* name = irGetArg1(line);
            out = kr_plus(kr_plus(kr_plus(out, kr_str("  %")), name), kr_str(".a = alloca ptr\n"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  store ptr %p")), paramIdx), kr_str(", ptr %")), name), kr_str(".a\n"));
            paramIdx = kr_plus(paramIdx, kr_str("1"));
        }
        if (kr_truthy(kr_eq(op, kr_str("LOCAL")))) {
            char* name = irGetArg1(line);
            out = kr_plus(kr_plus(kr_plus(out, kr_str("  %")), name), kr_str(".a = alloca ptr\n"));
            char* empReg = kr_plus(kr_str("%emp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(out, kr_str("  ")), empReg), kr_str(" = getelementptr inbounds [1 x i8], ptr @.empty, i32 0, i32 0\n"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  store ptr ")), empReg), kr_str(", ptr %")), name), kr_str(".a\n"));
        }
        if (kr_truthy(kr_eq(op, kr_str("PUSH")))) {
            char* val = kr_trim(irGetRest(line));
            char* r = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            if (kr_truthy(kr_startswith(val, kr_str("\"")))) {
                char* inner = kr_substr(val, kr_str("1"), kr_sub(kr_len(val), kr_str("1")));
                char* slen = kr_plus(kr_len(inner), kr_str("1"));
                out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), r), kr_str(" = getelementptr inbounds [")), slen), kr_str(" x i8], ptr @kp_")), fname), kr_str("_")), gidx), kr_str(", i32 0, i32 0\n"));
            } else {
                char* slen = kr_plus(kr_len(val), kr_str("1"));
                out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), r), kr_str(" = getelementptr inbounds [")), slen), kr_str(" x i8], ptr @kp_")), fname), kr_str("_")), gidx), kr_str(", i32 0, i32 0\n"));
            }
            gidx = kr_plus(gidx, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  store ptr ")), r), kr_str(", ptr %stk")), stkDepth), kr_str("\n"));
            stkDepth = kr_plus(stkDepth, kr_str("1"));
        }
        if (kr_truthy(kr_eq(op, kr_str("LOAD")))) {
            char* name = irGetArg1(line);
            char* r = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), r), kr_str(" = load ptr, ptr %")), name), kr_str(".a\n"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  store ptr ")), r), kr_str(", ptr %stk")), stkDepth), kr_str("\n"));
            stkDepth = kr_plus(stkDepth, kr_str("1"));
        }
        if (kr_truthy(kr_eq(op, kr_str("STORE")))) {
            char* name = irGetArg1(line);
            stkDepth = kr_sub(stkDepth, kr_str("1"));
            char* r = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), r), kr_str(" = load ptr, ptr %stk")), stkDepth), kr_str("\n"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  store ptr ")), r), kr_str(", ptr %")), name), kr_str(".a\n"));
        }
        if (kr_truthy((kr_truthy((kr_truthy((kr_truthy((kr_truthy((kr_truthy(kr_eq(op, kr_str("ADD"))) || kr_truthy(kr_eq(op, kr_str("SUB"))) ? kr_str("1") : kr_str("0"))) || kr_truthy(kr_eq(op, kr_str("MUL"))) ? kr_str("1") : kr_str("0"))) || kr_truthy(kr_eq(op, kr_str("DIV"))) ? kr_str("1") : kr_str("0"))) || kr_truthy(kr_eq(op, kr_str("MOD"))) ? kr_str("1") : kr_str("0"))) || kr_truthy(kr_eq(op, kr_str("CAT"))) ? kr_str("1") : kr_str("0")))) {
            stkDepth = kr_sub(stkDepth, kr_str("1"));
            char* rb = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), rb), kr_str(" = load ptr, ptr %stk")), stkDepth), kr_str("\n"));
            stkDepth = kr_sub(stkDepth, kr_str("1"));
            char* ra = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), ra), kr_str(" = load ptr, ptr %stk")), stkDepth), kr_str("\n"));
            char* fn2 = kr_str("kr_plus");
            if (kr_truthy(kr_eq(op, kr_str("SUB")))) {
                fn2 = kr_str("kr_sub");
            }
            if (kr_truthy(kr_eq(op, kr_str("MUL")))) {
                fn2 = kr_str("kr_mul");
            }
            if (kr_truthy(kr_eq(op, kr_str("DIV")))) {
                fn2 = kr_str("kr_div");
            }
            if (kr_truthy(kr_eq(op, kr_str("MOD")))) {
                fn2 = kr_str("kr_mod");
            }
            if (kr_truthy(kr_eq(op, kr_str("CAT")))) {
                fn2 = kr_str("kr_cat");
            }
            char* r = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), r), kr_str(" = call ptr @")), fn2), kr_str("(ptr ")), ra), kr_str(", ptr ")), rb), kr_str(")\n"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  store ptr ")), r), kr_str(", ptr %stk")), stkDepth), kr_str("\n"));
            stkDepth = kr_plus(stkDepth, kr_str("1"));
        }
        if (kr_truthy(kr_eq(op, kr_str("NEG")))) {
            stkDepth = kr_sub(stkDepth, kr_str("1"));
            char* ra = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), ra), kr_str(" = load ptr, ptr %stk")), stkDepth), kr_str("\n"));
            char* r = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), r), kr_str(" = call ptr @kr_neg(ptr ")), ra), kr_str(")\n"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  store ptr ")), r), kr_str(", ptr %stk")), stkDepth), kr_str("\n"));
            stkDepth = kr_plus(stkDepth, kr_str("1"));
        }
        if (kr_truthy(kr_eq(op, kr_str("NOT")))) {
            stkDepth = kr_sub(stkDepth, kr_str("1"));
            char* ra = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), ra), kr_str(" = load ptr, ptr %stk")), stkDepth), kr_str("\n"));
            char* r = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), r), kr_str(" = call ptr @kr_not(ptr ")), ra), kr_str(")\n"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  store ptr ")), r), kr_str(", ptr %stk")), stkDepth), kr_str("\n"));
            stkDepth = kr_plus(stkDepth, kr_str("1"));
        }
        if (kr_truthy((kr_truthy((kr_truthy((kr_truthy((kr_truthy((kr_truthy(kr_eq(op, kr_str("EQ"))) || kr_truthy(kr_eq(op, kr_str("NEQ"))) ? kr_str("1") : kr_str("0"))) || kr_truthy(kr_eq(op, kr_str("LT"))) ? kr_str("1") : kr_str("0"))) || kr_truthy(kr_eq(op, kr_str("GT"))) ? kr_str("1") : kr_str("0"))) || kr_truthy(kr_eq(op, kr_str("LTE"))) ? kr_str("1") : kr_str("0"))) || kr_truthy(kr_eq(op, kr_str("GTE"))) ? kr_str("1") : kr_str("0")))) {
            stkDepth = kr_sub(stkDepth, kr_str("1"));
            char* rb = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), rb), kr_str(" = load ptr, ptr %stk")), stkDepth), kr_str("\n"));
            ...
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), ra), kr_str(" = load ptr, ptr %stk")), stkDepth), kr_str("\n"));
            char* fn2 = kr_str("kr_eq");
            if (kr_truthy(kr_eq(op, kr_str("NEQ")))) {
                fn2 = kr_str("kr_neq");
            }
        ...
        if (kr_truthy((kr_truthy(kr_eq(op, kr_str("CALL"))) || kr_truthy(kr_eq(op, kr_str("BUILTIN"))) ? kr_str("1") : kr_str("0")))) {
            char* fname2 = irGetArg1(line);
            char* argc2 = kr_toint(irGetArg2(line));
            char* argTemps = kr_str("");
            char* ai = kr_sub(argc2, kr_str("1"));
            while (kr_truthy(kr_gte(ai, kr_str("0")))) {
                stkDepth = kr_sub(stkDepth, kr_str("1"));
                char* r = kr_plus(kr_str("%tmp"), reg);
                reg = kr_plus(reg, kr_str("1"));
                out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), r), kr_str(" = load ptr, ptr %stk")), stkDepth), kr_str("\n"));
                if (kr_truthy(kr_eq(ai, kr_str("0")))) {
                    argTemps = kr_plus(kr_plus(r, kr_str(",")), argTemps);
                } else {
                    argTemps = kr_plus(kr_plus(r, kr_str(",")), argTemps);
                }
                ai = kr_sub(ai, kr_str("1"));
            }
            char* result = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            char* callArgs = kr_str("");
            char* bi = kr_str("0");
            while (kr_truthy(kr_lt(bi, argc2))) {
                char* argR = kr_split(argTemps, bi);
                if (kr_truthy(kr_gt(bi, kr_str("0")))) {
                    callArgs = kr_plus(callArgs, kr_str(", "));
                }
                callArgs = kr_plus(kr_plus(callArgs, kr_str("ptr ")), argR);
                bi = kr_plus(bi, kr_str("1"));
            }
            char* rfname = fname2;
            if (kr_truthy(kr_eq(fname2, kr_str("kp")))) {
                rfname = kr_str("kr_kp");
            }
            if (kr_truthy(kr_eq(fname2, kr_str("print")))) {
                rfname = kr_str("kr_print");
            }
            if (kr_truthy(kr_eq(fname2, kr_str("len")))) {
                rfname = kr_str("kr_len");
            }
            if (kr_truthy(kr_eq(fname2, kr_str("toInt")))) {
                rfname = kr_str("kr_toInt");
            }
            if (kr_truthy(kr_eq(fname2, kr_str("contains")))) {
                rfname = kr_str("kr_contains");
            }
            if (kr_truthy(kr_eq(fname2, kr_str("startsWith")))) {
                rfname = kr_str("kr_startsWith");
            }
            if (kr_truthy(kr_eq(fname2, kr_str("fadd")))) {
                rfname = kr_str("kr_fadd");
            }
            if (kr_truthy(kr_eq(fname2, kr_str("fsub")))) {
                rfname = kr_str("kr_fsub");
            }
            if (kr_truthy(kr_eq(fname2, kr_str("fmul")))) {
                rfname = kr_str("kr_fmul");
            }
            if (kr_truthy(kr_eq(fname2, kr_str("fdiv")))) {
                rfname = kr_str("kr_fdiv");
            }
            if (kr_truthy(kr_eq(fname2, kr_str("fsqrt")))) {
                rfname = kr_str("kr_fsqrt");
            }
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), result), kr_str(" = call ptr @")), rfname), kr_str("(")), callArgs), kr_str(")\n"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  store ptr ")), result), kr_str(", ptr %stk")), stkDepth), kr_str("\n"));
            stkDepth = kr_plus(stkDepth, kr_str("1"));
        }
        if (kr_truthy(kr_eq(op, kr_str("POP")))) {
            stkDepth = kr_sub(stkDepth, kr_str("1"));
        }
        if (kr_truthy(kr_eq(op, kr_str("RETURN")))) {
            stkDepth = kr_sub(stkDepth, kr_str("1"));
            char* r = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), r), kr_str(" = load ptr, ptr %stk")), stkDepth), kr_str("\n"));
            out = kr_plus(kr_plus(kr_plus(out, kr_str("  ret ptr ")), r), kr_str("\n"));
            lastWasTerm = kr_str("1");
        }
        if (kr_truthy(kr_eq(op, kr_str("LABEL")))) {
            char* lname = irGetArg1(line);
            if (kr_truthy(kr_eq(lastWasTerm, kr_str("0")))) {
                out = kr_plus(kr_plus(kr_plus(out, kr_str("  br label %")), lname), kr_str("\n"));
            }
            out = kr_plus(kr_plus(out, lname), kr_str(":\n"));
            lastWasTerm = kr_str("0");
        }
        if (kr_truthy(kr_eq(op, kr_str("JUMP")))) {
            char* target = irGetArg1(line);
            out = kr_plus(kr_plus(kr_plus(out, kr_str("  br label %")), target), kr_str("\n"));
            lastWasTerm = kr_str("1");
        }
        if (kr_truthy((kr_truthy(kr_eq(op, kr_str("JUMPIF"))) || kr_truthy(kr_eq(op, kr_str("JUMPIFNOT"))) ? kr_str("1") : kr_str("0")))) {
            char* target = irGetArg1(line);
            stkDepth = kr_sub(stkDepth, kr_str("1"));
            char* rv = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), rv), kr_str(" = load ptr, ptr %stk")), stkDepth), kr_str("\n"));
            char* cond = kr_plus(kr_str("%cond"), reg);
            reg = kr_plus(reg, kr_str("1"));
            char* ft = kr_plus(kr_str("_ft"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  ")), cond), kr_str(" = call i1 @kr_truthy(ptr ")), rv), kr_str(")\n"));
            if (kr_truthy(kr_eq(op, kr_str("JUMPIF")))) {
                out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  br i1 ")), cond), kr_str(", label %")), target), kr_str(", label %")), ft), kr_str("\n"));
            } else {
                out = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(out, kr_str("  br i1 ")), cond), kr_str(", label %")), ft), kr_str(", label %")), target), kr_str("\n"));
            }
            out = kr_plus(kr_plus(out, ft), kr_str(":\n"));
            lastWasTerm = kr_str("0");
        }
        if (kr_truthy(kr_eq(op, kr_str("END")))) {
            char* empR = kr_plus(kr_str("%tmp"), reg);
            reg = kr_plus(reg, kr_str("1"));
            out = kr_plus(kr_plus(kr_plus(out, kr_str("  ")), empR), kr_str(" = getelementptr inbounds [1 x i8], ptr @.empty, i32 0, i32 0\n"));
            out = kr_plus(kr_plus(kr_plus(out, kr_str("  ret ptr ")), empR), kr_str("\n"));
            out = kr_plus(out, kr_str("}\n\n"));
        }
        i = kr_plus(i, kr_str("1"));
    }
...
int main(int argc, char** argv) {
    ...
    if (kr_truthy(kr_eq(kr_len(ir), kr_str("0")))) {
        kr_printerr(kr_plus(kr_str("llvm: cannot read: "), file));
        return 0;
    }
    kr_print(kr_str("; Krypton LLVM IR v3 — opaque pointers"));
    kr_print(kr_str(""));
    kr_print(kr_str("declare ptr @kr_plus(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_sub(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_mul(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_div(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_mod(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_neg(ptr)"));
    kr_print(kr_str("declare ptr @kr_not(ptr)"));
    kr_print(kr_str("declare ptr @kr_cat(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_eq(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_neq(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_lt(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_gt(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_lte(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_gte(ptr, ptr)"));
    kr_print(kr_str("declare i1  @kr_truthy(ptr)"));
    kr_print(kr_str("declare ptr @kr_kp(ptr)"));
    kr_print(kr_str("declare ptr @kr_print(ptr)"));
    kr_print(kr_str("declare ptr @kr_len(ptr)"));
    kr_print(kr_str("declare ptr @kr_toInt(ptr)"));
    kr_print(kr_str("declare ptr @kr_contains(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_startsWith(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_fadd(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_fsub(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_fmul(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_fdiv(ptr, ptr)"));
    kr_print(kr_str("declare ptr @kr_fsqrt(ptr)"));
    kr_print(kr_str(""));
    kr_print(kr_str("@.empty = private unnamed_addr constant [1 x i8] zeroinitializer"));
    kr_print(kr_str(""));
    char* allLines = ir;
    char* total = kr_linecount(allLines);
    char* clean = kr_str("");
    char* ci = kr_str("0");
    while (kr_truthy(kr_lt(ci, total))) {
        char* ln = kr_getline(allLines, ci);
        char* tln = kr_trim(ln);
        if (kr_truthy(kr_gt(kr_len(tln), kr_str("0")))) {
            if (kr_truthy((kr_truthy((kr_truthy(kr_neq(kr_idx(tln, kr_atoi(kr_str("0"))), kr_str(";"))) && kr_truthy(kr_neq(kr_idx(tln, kr_atoi(kr_str("0"))), kr_str("#"))) ? kr_str("1") : kr_str("0"))) && kr_truthy(kr_neq(kr_idx(tln, kr_atoi(kr_str("0"))), kr_str("/"))) ? kr_str("1") : kr_str("0")))) {
                if (kr_truthy(kr_gt(kr_len(clean), kr_str("0")))) {
                    clean = kr_plus(clean, kr_str("\n"));
                }
                clean = kr_plus(clean, tln);
            }
        }
        ci = kr_plus(ci, kr_str("1"));
    }
    char* blines = clean;
    char* bcount = kr_linecount(blines);
    char* i = kr_str("0");
    while (kr_truthy(kr_lt(i, bcount))) {
        char* line = kr_getline(blines, i);
        char* op = irGetOp(line);
        if (kr_truthy(kr_eq(op, kr_str("FUNC")))) {
            char* fname = irGetArg1(line);
            char* argc = irGetArg2(line);
            char* body = kr_str("");
            i = kr_plus(i, kr_str("1"));
            while (kr_truthy(kr_lt(i, bcount))) {
                char* bl = kr_getline(blines, i);
                if (kr_truthy(kr_eq(irGetOp(bl), kr_str("END")))) {
                    if (kr_truthy(kr_gt(kr_len(body), kr_str("0")))) {
                        body = kr_plus(body, kr_str("\n"));
                    }
                    body = kr_plus(body, bl);
                    i = kr_plus(i, kr_str("1"));
                    break;
                }
                if (kr_truthy(kr_gt(kr_len(body), kr_str("0")))) {
            ...
            kr_print(emitGlobalsForFunc(fname, body));
            kr_print(emitFunc(fname, argc, body));
        } else {
            i = kr_plus(i, kr_str("1"));
        }
    }

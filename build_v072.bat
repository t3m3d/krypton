...
setlocal
echo ============================================================
echo  Krypton v0.7.2 Build Script
echo ============================================================

set KCC=versions\kcc_v071.exe
set COMPILE=kompiler\compile.k
set OUT_C=compile_out.c
set OUT_EXE=versions\kcc_v072.exe

echo.
echo [1/5] Verifying compiler on features_test.k...
...
echo.
echo [5/5] Running v0.7.2 tests...
%OUT_EXE% %COMPILE% test_v072.k > test_v072_out.c
if errorlevel 1 ( echo FAILED & pause & exit /b 1 )
gcc test_v072_out.c -o test_v072.exe -w
if errorlevel 1 ( echo FAILED & pause & exit /b 1 )
test_v072.exe

echo.
echo Done! kcc_v072.exe built and tested.
pause
endlocal
